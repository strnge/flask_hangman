...
def new_game():
    ...
    return render_template('index.html', display_word=session["display_word"], state=session["debug_state"], health=session["health"], wins=session["wins"], losses=session["losses"], graveyard=session["graveyard"])
...
def guess():
    ...
    request_data = request.args["input_box"]
    return render_template('index.html', display_word=session["display_word"], state=session["debug_state"], health=session["health"], wins=session["wins"], losses=session["losses"], graveyard=session["graveyard"])
